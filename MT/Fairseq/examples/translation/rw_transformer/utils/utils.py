...
from examples.translation.rw_transformer.modules.rw_attention import DisentangledAttention
...
def plot_attention_map(attn: torch.Tensor,src_tokens, tgt_tokens, tgt_label="auto", src_label="auto", save=""):
    """
        attn: [heads, layers, tgt, src]
    """
    attn = attn.detach().cpu()
    layers = attn.shape[1]
    num=attn[:, 0].mean(0).numpy()
    io.savemat('/root/Disentangled-Mask-Attention-in-Transformers/MT/Fairseq/checkpoints/wmt14_en_de/transformer/test.mat',{'attn':num})
    plt.figure(figsize=(10, 10))
    for layer in range(layers):
        if layer==0 :
            #plt.subplot(math.ceil(layers/2), 2, layer+1)
            src_tokens_tmp=src_tokens[0]
            sns.heatmap(attn[:, layer].mean(0),xticklabels=src_label, yticklabels=tgt_label)#平均每個head的attention map
            print(src_tokens_tmp)
            #sns.heatmap(attn[:, layer].mean(0), xticklabels=src_tokens_tmp, yticklabels=src_tokens_tmp)
...
def plot_token_distribution_noc(embeddings_enc: torch.Tensor,embeddings_dec: torch.Tensor,pos_enc_dec: torch.Tensor,
                            save: str=""):
    """
        embeddings: [tokens, dims]
        head_idx: [clusters,]
    ...
    """
    sns.set_style("white")
    len_enc=torch.arange(0,len(embeddings_enc))
    len_dec=torch.arange(0,len(embeddings_dec))
    pos_index=torch.cat((len_enc,len_dec),0)
    pos_index =  pos_index.int().numpy().reshape(-1, 1)

    pos_tmp=[]
    for i in (range(len(embeddings_enc))):
        pos_tmp.append("enc_pos")
    for i in (range(len(embeddings_dec))):
        pos_tmp.append("dec_pos")

    pos_enc_dec = pos_enc_dec.int().numpy().reshape(-1, 1)
    clusters = pos_enc_dec.max() + 1
    embeddings=torch.cat((embeddings_enc,embeddings_dec),0)

    tsne = TSNE(n_components=2)    
    tsne_results = tsne.fit_transform(embeddings)

    df_result = pd.DataFrame(
        np.concatenate([tsne_results,pos_index], 1),
        columns=(["x", "y","pos_index"])
    )
    df_result["pos_label"]=pos_tmp

    f, ax = plt.subplots(figsize=(9, 6))
    pal = dict({"enc_pos": "green", "dec_pos": "orange"})
    ...
    ax = sns.scatterplot(
        ...
        hue="pos_label",
        style="pos_label",
        data=df_result.iloc[:, :4], # Data source
        size="pos_index",
        size_order=df_result['pos_index'],
        palette=pal,
        ax=ax
    ) # size and dimension
