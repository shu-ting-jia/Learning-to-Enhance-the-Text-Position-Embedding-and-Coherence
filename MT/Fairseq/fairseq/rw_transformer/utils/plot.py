...
def plot_attention(outputs_dict, src_lens, tgt_lens ,src_tokens, tgt_tokens,enc=False, dec=False, dec_enc=False,
                         batch=0, figure_dir="" ):
    assert enc or dec or dec_enc

    if enc:
        attn = outputs_dict["enc_mask_attn"][0, :, :, :src_lens[batch], :src_lens[batch]]
    elif dec:
        attn = outputs_dict["dec_mask_attn"][0, :, :, :tgt_lens[batch], :tgt_lens[batch]]
    else:
        attn = outputs_dict["dec_enc_mask_attn"][0, :, :, :tgt_lens[batch], :src_lens[batch]]
    attn = attn.squeeze(0).contiguous()
    print(attn.size())
    #plot_attention_map(attn, save=figure_dir) # (LxH) x (LxH)
    plot_attention_map(attn,src_tokens, tgt_tokens , save=figure_dir) # (LxH) x (LxH)


def plot_position_embedding(outputs_dict, src_lens, tgt_lens, enc=False, dec=False, dec_enc=False,
                         batch=0, figure_dir=""):
    assert enc or dec 

    
    embeddings_enc = outputs_dict["pos_emb"].float().mean(0)

    embeddings_dec = outputs_dict["dec_pos"].float().mean(0) 
    
    pos_enc_len = torch.zeros_like(embeddings_enc.mean(1))
    pos_dec_len = torch.ones_like(embeddings_dec.mean(1)) 
    pos_enc_dec_len=torch.cat((pos_enc_len,pos_dec_len),0)

    print("\tEnc_Samples: ", len(embeddings_enc))
    #embeddings = embeddings.reshape(-1, embeddings.shape[-1])
    print("\tDnc_Samples: ", len(embeddings_dec))

    plot_token_distribution_noc(
        embeddings_enc,
        embeddings_dec,
        pos_enc_dec_len,
        save=os.path.join(figure_dir, "pos_tsne.png") if figure_dir else ""
    )
...
def main():
    ...
    batch_data = get_batch_data_with_length_limited(model, iterator, min_length=36)
    ...
    if args.head_redundancy or args.all:
        print("\nPlotting attention enc...")
        plot_attention(outputs_dict, src_lens, tgt_lens,src_tokens, tgt_tokens, enc=True,figure_dir=figure)
        print("\nPlotting position  ...")
        #plot_position_embedding(outputs_dict, src_lens, tgt_lens, dec=True, figure_dir=figure)
